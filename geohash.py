...
def _fixedpoint(num, bound_max, bound_min):
    ...
    return float(decimal.Decimal(num))


@annotate("string->struct<sw:struct<lat:double,lon:double>,ne:struct<lat:double,lon:double>>")
class bounds(object):
    def evaluate(self, geohash):
        """
...
@annotate("string->struct<lat:double,lon:double>")
class decode(object):
    def evaluate(self, geohash):
        """
        ...
        >>> (70.2995, -27.9993)
        """
        (lat_min, lon_min), (lat_max, lon_max) = bounds().evaluate(geohash)
...
class encode(object):
    def evaluate(self, lat, lon, precision):
        """
...
class adjacent(object):
    def evaluate(self, geohash, direction):
        """
        ...
        if last_char in border[direction][typ] and parent:
            parent = adjacent().evaluate(parent, direction)

        index = neighbour[direction][typ].index(last_char)
        return parent + base32[index]


@annotate("string->struct<n:string,ne:string,e:string,se:string,s:string,sw:string,w:string,nw:string>")
class neighbours(object):
    def evaluate(self, geohash):
        ...
        >>> gcpuypm
        """
        n = adjacent().evaluate(geohash, 'n')
        ne = adjacent().evaluate(n, 'e')
        e = adjacent().evaluate(geohash, 'e')
        s = adjacent().evaluate(geohash, 's')
        se = adjacent().evaluate(s, 'e')
        w = adjacent().evaluate(geohash, 'w')
        sw = adjacent().evaluate(s, 'w')
        nw = adjacent().evaluate(n, 'w')
        Neighbours = namedtuple('Neighbours',
                                ['n', 'ne', 'e', 'se', 's', 'sw', 'w', 'nw'])
        return Neighbours(n, ne, e, se, s, sw, w, nw)

print(neighbours().evaluate('gcpuyph'))